...
<body class="auth-page-body">
    ...
    <div class="auth-card">
        ...
        <h1 class="auth-title">{{ __('messages.auth.create_account') }}</h1>
        <p class="auth-sub">{{ __('messages.auth.get_started') }}</p>
        ...
        <form method="POST" action="{{ route('register') }}">
            @csrf
            <div class="mb-3">
                <label for="name" class="form-label fw-medium">{{ __('messages.auth.name') }}</label>
                <input type="text" id="name" name="name" class="form-control" value="{{ old('name') }}" placeholder="{{ __('messages.auth.name') }}" required autofocus>
            </div>
            <div class="mb-3">
                <label for="email" class="form-label fw-medium">{{ __('messages.auth.email') }}</label>
                <input type="email" id="email" name="email" class="form-control" value="{{ old('email') }}" placeholder="{{ __('messages.auth.email') }}" required>
            </div>
            <div class="mb-3">
                <label for="password" class="form-label fw-medium">{{ __('messages.auth.password') }}</label>
                <input type="password" id="password" name="password" class="form-control" placeholder="{{ __('messages.auth.password') }}" required>
                <div class="form-text">{{ __('messages.auth.password_hint') }}</div>
            </div>
            <div class="mb-3">
                <label for="password_confirmation" class="form-label fw-medium">{{ __('messages.auth.confirm_password') }}</label>
                <input type="password" id="password_confirmation" name="password_confirmation" class="form-control" placeholder="{{ __('messages.auth.confirm_password') }}" required>
            </div>
            <button type="submit" class="btn btn-primary mb-3">{{ __('messages.auth.create_account') }}</button>
        </form>
        ...
        <div class="d-flex flex-column gap-2">
            <a href="{{ route('auth.google') }}" class="btn btn-social btn-google">
                ...
                {{ __('messages.auth.continue_google') }}
            </a>
        ...
        <p class="auth-footer mt-3">{{ __('messages.auth.has_account') }} <a href="{{ route('auth.login') }}">{{ __('messages.nav.sign_in') }}</a></p>
    </div>
