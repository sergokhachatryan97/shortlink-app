...
<head>
    ...
    <style>
        ...
        .auth-card .form-check-label { color: rgba(255,255,255,0.8); }
        .auth-card .form-check-input { background-color: rgba(15,15,25,0.8); border-color: rgba(255,255,255,0.3); }
...
<body class="auth-page-body">
    ...
    <div class="auth-card">
        ...
        <p class="auth-sub">{{ __('messages.auth.sign_in_sub') }}</p>
        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            <div class="mb-3">
                ...
                <input type="email" id="email" name="email" class="form-control" value="{{ old('email') }}" placeholder="{{ __('messages.auth.email') }}" required autofocus>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3 form-check">
                ...
                <label for="remember" class="form-check-label small">{{ __('messages.auth.remember') }}</label>
            </div>
            <button type="submit" class="btn btn-primary mb-3">{{ __('messages.auth.sign_in_btn') }}</button>
        </form>

        <div class="divider">{{ __('messages.auth.or_continue') }}</div>
        ...
        <div class="d-flex flex-column gap-2">
            ...
            <a href="#" class="btn btn-social btn-telegram" style="opacity: 0.7; pointer-events: none;" title="Set TELEGRAM_BOT_USERNAME in .env">
                ...
                {{ __('messages.auth.continue_telegram') }}
            </a>
            @endif
        </div>

        <p class="auth-footer mt-3">{{ __('messages.auth.no_account') }} <a href="{{ route('auth.register') }}">{{ __('messages.nav.sign_up') }}</a></p>
    </div>
